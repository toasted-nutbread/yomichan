...
class Translator {
    ...
    *_getArrayVariants(arrayVariants) {
    ...
    _createTermDefinition(index, headwordIndices, dictionary, dictionaryIndex, dictionaryPriority, score, sequences, isPrimary, tags, entries) {
        return {index, headwordIndices, dictionary, dictionaryIndex, dictionaryPriority, score, sequences, isPrimary, tags, entries};
    }
    ...
    _createTermDictionaryEntryFromDatabaseEntry(databaseEntry, originalText, transformedText, deinflectedText, reasons, isPrimary, enabledDictionaryMap) {
        ...
        return this._createTermDictionaryEntry(
            ...
            [this._createTermDefinition(0, [0], dictionary, dictionaryIndex, dictionaryPriority, score, [sequence], isPrimary, definitionTagGroups, definitions)]
        );
    }
    ...
    _addTermDefinitionsFast(definitions, newDefinitions, headwordIndexMap) {
        for (const {headwordIndices, dictionary, dictionaryIndex, dictionaryPriority, sequences, score, isPrimary, tags, entries} of newDefinitions) {
            const headwordIndicesNew = [];
            for (const headwordIndex of headwordIndices) {
                headwordIndicesNew.push(headwordIndexMap[headwordIndex]);
            }
            definitions.push(this._createTermDefinition(definitions.length, headwordIndicesNew, dictionary, dictionaryIndex, dictionaryPriority, score, sequences, isPrimary, tags, entries));
        }
    }
    ...
    _addTermDefinitions(definitions, definitionsMap, newDefinitions, headwordIndexMap) {
        for (const {headwordIndices, dictionary, dictionaryIndex, dictionaryPriority, sequences, score, isPrimary, tags, entries} of newDefinitions) {
            ...
            if (typeof definition === 'undefined') {
                definition = this._createTermDefinition(definitions.length, [], dictionary, dictionaryIndex, dictionaryPriority, score, [...sequences], isPrimary, [], [...entries]);
