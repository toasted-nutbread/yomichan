...
async function testDomTextScanner(dom, {DOMTextScanner}) {
    ...
    for (const testElement of document.querySelectorAll('.test[data-test-data]')) {
        ...
        for (const testDataItem of testData) {
            ...
            // Standard test
            {
                const scanner = new DOMTextScanner(node, offset);
                scanner.seek(length);
            ...
            // Reversed test
            {
                const scanner = new DOMTextScanner(expectedNode, expectedOffset);
                scanner.seek(-length);
